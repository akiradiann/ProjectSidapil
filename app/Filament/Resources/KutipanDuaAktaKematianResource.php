...
use App\Filament\Resources\KutipanDuaAktaKematianResource\Pages;
use App\Models\KutipanDuaAktaKematian;
use App\Models\User;
...
class KutipanDuaAktaKematianResource extends Resource
{
    protected static ?string $model = KutipanDuaAktaKematian::class;

    protected static ?string $navigationIcon = 'heroicon-o-document-duplicate';

    protected static ?string $navigationLabel = 'Kutipan Dua Akta Kematian';

    protected static ?string $modelLabel = 'Kutipan Dua Akta Kematian';

    protected static ?string $pluralModelLabel = 'Kutipan Dua Akta Kematian';
    ...
    public static function form(Form $form): Form
    {
        ...
        return $form
            ->schema([
                Forms\Components\Section::make('Informasi Kutipan Dua Akta Kematian')
                    ->description('Data kutipan dua akta kematian')
                    ->schema([
                        ...
                        Forms\Components\Select::make('kode')
                            ->label('Kode')
                            ->options(KutipanDuaAktaKematian::getKodeOptions())
                            ->required()
                            ->searchable()
                        ...
                        Forms\Components\Select::make('no_akta')
                            ...
                            ->afterStateUpdated(function ($state, Forms\Set $set) {
                                if ($state) {
                                    $ref = \App\Models\AktaKematian::where('nomor', $state)->first();
                                    if ($ref) {
                                        $set('nama', $ref->nama);
                ...
                Forms\Components\Section::make('Informasi Pelapor')
                    ...
                    ->schema([
                        Forms\Components\CheckboxList::make('serviceRequest.checklist_persyaratan')
                            ->label('Persyaratan')
                            ->options([
                                    'Kartu Keluarga Pemohon/Keluarga Almarhum' => 'Kartu Keluarga Pemohon/Keluarga Almarhum',
                ...
                Forms\Components\Section::make('Status & Produk')
                    ...
                    ->schema([
                        ...
                        Forms\Components\FileUpload::make('file_produk')
                            ...
                            ->directory('kutipan-dua-akta-kematian/files')
                            ->preserveFilenames()
                            ->disk('local')
                            ->visibility('private')
                            ->downloadable()
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->emptyStateHeading('Belum ada kutipan dua akta kematian')
            ->emptyStateDescription('Mulai dengan membuat kutipan dua akta kematian baru.')
            ->emptyStateActions([
    ...
    {
        return [
            'index' => Pages\ListKutipanDuaAktaKematians::route('/'),
            'create' => Pages\CreateKutipanDuaAktaKematian::route('/create'),
            'view' => Pages\ViewKutipanDuaAktaKematian::route('/{record}'),
            'edit' => Pages\EditKutipanDuaAktaKematian::route('/{record}/edit'),
        ];
    }
}
