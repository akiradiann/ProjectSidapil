...
use App\Filament\Resources\AktaPerkawinanResource\Pages;
...
class AktaPerkawinanResource extends Resource
{
    protected static ?string $model = AktaPerkawinan::class;

    protected static ?string $navigationIcon = 'heroicon-o-document-duplicate';

    protected static ?string $navigationLabel = 'Akta Perkawinan';

    protected static ?string $modelLabel = 'Akta Perkawinan';

    protected static ?string $pluralModelLabel = 'Akta Perkawinan';
    ...
    public static function form(Form $form): Form
    {
        ...
        return $form
            ->schema([
                Forms\Components\Section::make('Informasi Akta Perkawinan')
                    ->description('Data akta perkawinan')
                    ->schema([
                        ...
                        Forms\Components\TextInput::make('kode')
                            ...
                            ->disabled()
                            ->dehydrated()
                            ->default(fn() => 'KW/' . date('Y'))
                            ->helperText('Kode otomatis: KW/{tahun}'),
                        Forms\Components\TextInput::make('nama_mempelai_laki')
                            ->label('Nama Mempelai Laki-Laki')
                            ->required()
                            ->maxLength(255)
                            ->disabled(fn($record) => $record && !$isFrontOffice && !$isAdmin && !$isOperator),
                        Forms\Components\TextInput::make('nama_mempelai_perempuan')
                            ->label('Nama Mempelai Perempuan')
                            ->required()
                            ->maxLength(255)
                            ->disabled(fn($record) => $record && !$isFrontOffice && !$isAdmin && !$isOperator),
                        Forms\Components\TextInput::make('tempat_perkawinan_agama')
                ...
                Forms\Components\Section::make('Status & Produk')
                    ...
                    ->schema([
                        ...
                        Forms\Components\FileUpload::make('file_produk')
                            ...
                            ->directory('akta-perkawinan/files')
                            ->preserveFilenames()
                            ->disk('local')
                            ->visibility('private')
                            ->downloadable()
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('nama_mempelai_laki')
                    ...
                    ->sortable()
                    ->weight('medium'),
                Tables\Columns\TextColumn::make('nama_mempelai_perempuan')
                    ->label('Nama Mempelai Perempuan')
                    ->searchable()
                    ->sortable()
                    ->weight('medium'),
                Tables\Columns\TextColumn::make('tanggal_perkawinan')
                    ->label('Tanggal Perkawinan')
                    ->date('d/m/Y')
                    ->sortable(),
    ...
    {
        return [
            'index' => Pages\ListAktaPerkawinan::route('/'),
            'create' => Pages\CreateAktaPerkawinan::route('/create'),
            'view' => Pages\ViewAktaPerkawinan::route('/{record}'),
            'edit' => Pages\EditAktaPerkawinan::route('/{record}/edit'),
        ];
    }
}
